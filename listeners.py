...
class Listeners(commands.Cog):
	...
	async def ready(self):
		botdata.logger.info('Bot iniciado')
		await self.bot.change_presence(activity=discord.Game(name=botdata.default_prefix+'help'))


	@commands.Cog.listener('on_resumed')
	async def resume(self):
		botdata.logger.info('Sesión resumida')
		await self.bot.change_presence(activity=discord.Game(name=botdata.default_prefix+'help'))
	...
	async def error_handler(self, ctx, error):
		...
		errors = {
			...
			commands.MaxConcurrencyReached: lambda: f'Se alcanzó el límite de usuarios para este comando: `{error.number}{"" if error.per == commands.BucketType.default else f" por {botdata.bucket_types[error.per]}"}`',
		...
		if isinstance(error, exceptions.BlacklistUserError):
			await ctx.author.send(botdata.Warning.error('Estás en la lista negra. No tienes permitido usar el bot'), delete_after=60)
			return

		elif isinstance(error, commands.CommandOnCooldown):
			retry_after = error.retry_after
			error_msg = f'Tienes que esperar **{botdata.fix_delta(timedelta(seconds=retry_after), ms=True)}** para ejecutar este comando de nuevo'
		...
		if error_msg == None:
			...
			).set_author(name=ctx.author.name, icon_url=ctx.author.avatar_url).set_footer(text='Este error ha sido notificado y será investigado para su posterior correción.'))
			raise error
		...
		if error_msg == '':
			...
			await self.send(ctx, botdata.Warning.error(error_msg), failed=True)
	...
	async def prefix_help(self, message):
		if message.content.replace('!', '') == f'<@{self.bot.user.id}>' and not message.author.bot:
			try: 
				botdata.check_blacklist(await self.bot.get_context(message))
				await message.channel.send(botdata.Warning.info(f'Hola, mi prefijo en este servidor es `{botdata.get_prefix(self.bot, message, ignore_mention=True)}`'))
			except exceptions.BlacklistUserError:
				await message.author.send(botdata.Warning.error('Estás en la lista negra. No tienes permitido usar el bot'), delete_after=30)				
	...
	async def command_logging(self, ctx):
		botdata.logger.info(f'Se he usado un comando: "{ctx.message.content}" {repr(ctx.message)}')

		if botdata.bot_mode == 'stable' and not ctx.command_failed:
			botdata.cursor.execute(f"SELECT * FROM COMMANDSTATS WHERE COMMAND='{ctx.command.name}'")
			command = botdata.cursor.fetchall()
			if command == []:
				botdata.cursor.execute(f"INSERT INTO COMMANDSTATS VALUES('{ctx.command.name}', 1)")

			else:
				botdata.cursor.execute(f"UPDATE COMMANDSTATS SET USES={command[0][1] + 1} WHERE COMMAND='{command[0][0]}'")
	...
	async def guild_join_logging(self, guild):
		botdata.logger.info(f'El bot ha salido de un servidor: {repr(guild)}')
