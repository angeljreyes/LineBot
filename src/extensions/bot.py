...
class About(commands.Cog):
    ...
    async def _ping(self, interaction: discord.Interaction):
        """Muestra en milisegundos lo que tarda el bot en enviar un mensaje desde que mandaste
        el comando
        """
    ...
    async def changelog(self, interaction: discord.Interaction, version: str = core.bot_version):
        ...
        if version == 'list':
            ...
            embed = (
                discord.Embed(title='Changelog', colour=db.default_color(interaction))
            ...
            if summary is None:
                await interaction.response.send_message(
                    core.Warning.error('Versión inválida'),
                    ephemeral=True,
                )
                return
    ...
    async def color(self, interaction: discord.Interaction, value: str = ''):
        ...
        if check is None:
            ...
            db.cursor.execute(
                'UPDATE colors SET value=? WHERE id=?',
    ...
    async def stats(self, interaction: discord.Interaction):
        ...
        embed = (
            discord.Embed(title='Información de Line Bot', colour=db.default_color(interaction))
    ...
    async def commandstats(self, interaction: discord.Interaction, command: str | None):
        ...
        if command is None:
            ...
            pages = pagination.Page.from_list(
                interaction,
                'Comandos más usados (Desde 27/06/2020)',
                f_stats,
            )
            ...
            if stats is None:
                await interaction.response.send_message(
                    core.Warning.error('El comando que escribiste no existe o no se ha usado'),
                    ephemeral=True,
                )
                return
            ...
            await interaction.response.send_message(
                core.Warning.info(
                    f'`/{command_name}` se ha usado {uses} {"veces" if stats[1] != 1 else "vez"}',
                ),
            )
