...
import exceptions
import pagination
from tags import TagContext, TagSafeInteraction
...
class TagsCog(
    ...
    async def toggle(self, interaction: discord.Interaction[commands.Bot]):
        ...
        if check is None:
            question_msg = 'Los tags en este servidor están desactivados. ¿Quieres activarlos?'
        else:
            question_msg = 'Los tags en este servidor están activados. ¿Quieres desactivarlos?'
        ...
        if check is None:
            if confirmation.value:
                db.cursor.execute('INSERT INTO tagsenabled VALUES(?)', (tag_interaction.guild_id,))
                content = core.Warning.success('Se activaron los tags en este servidor')

            else:
                content = core.Warning.cancel('No se activarán los tags en este servidor')
            ...
            if confirmation.value:
                ...
                content = core.Warning.success('Se desactivaron los tags en este servidor')

            else:
                content = core.Warning.cancel('No se desactivarán los tags en este servidor')
    ...
    async def add(
        ...
        await interaction.response.send_message(
            core.Warning.success(f'Se agregó el tag `{clean_name}`'),
        )
    ...
    async def gift(
        ...
        if gift_permission.value:
            ...
            content = core.Warning.cancel(f'{interaction.user.mention} El regalo fue rechazado')
    ...
    async def rename(
        ...
        await interaction.response.send_message(core.Warning.success(
            f'El nombre del tag `{clean_old_name}` ha sido cambiado a `{clean_new_name}`',
        ))
    ...
    async def edit(
        ...
        if interaction.user != tag.user:
            await interaction.response.send_message(
                core.Warning.error('No puedes editar tags de otros usuarios'),
                ephemeral=True,
            )
            return
        ...
        await interaction.response.send_message(
            core.Warning.success(f'Se editó el tag `{clean_tag_name}`'),
        )
    ...
    async def delete(
        ...
        if interaction.user != tag.user:
            await interaction.response.send_message(
                core.Warning.error('No puedes eliminar tags de otros usuarios'),
                ephemeral=True,
            )
            return
        ...
        clean_tag_name = await commands.clean_content().convert(ctx, tag_name)
        await interaction.response.send_message(
            core.Warning.question(f'¿Quieres eliminar el tag `{clean_tag_name}`?'),
            view=confirmation,
        )
        await confirmation.wait()
        ...
        if confirmation.value:
            tag.delete()
            await confirmation.last_interaction.response.edit_message(
                content=core.Warning.success(f'El tag `{clean_tag_name}` ha sido eliminado'),
                view=confirmation,
            )
    ...
    async def forcedelete(
        ...
        clean_tag_name = await commands.clean_content().convert(ctx, tag_name)
        await interaction.response.send_message(
            core.Warning.success(f'El tag `{clean_tag_name}` ha sido eliminado'),
            ephemeral=silent,
        )
    ...
    async def owner(
        ...
        clean_tag_name = await commands.clean_content().convert(ctx, tag.name)
        await interaction.response.send_message(
            core.Warning.info(f'El dueño del tag `{clean_tag_name}` es **{str(tag.user)}**'),
        )
