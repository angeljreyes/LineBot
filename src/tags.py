...
class Tag:
    ...
    def gift(self, user: discord.Member) -> None:
        """Set the tag's owner."""
        db.cursor.execute(
            'UPDATE tags SET user=? WHERE guild=? AND name=?',
            (user.id, self.guild.id, self.name),
        )
        db.conn.commit()
        self.user = user
    ...
    def rename(self, name: str) -> None:
        """Set the tag's name."""
        db.cursor.execute(
            'UPDATE tags SET name=? WHERE guild=? AND name=?',
            (name, self.guild.id, self.name),
        )
        db.conn.commit()
        self.name = name
    ...
    def edit(self, content: str, nsfw: bool) -> None:
        ...
        self.nsfw = nsfw
        db.cursor.execute(
            'UPDATE tags SET content=?, nsfw=? WHERE guild=? AND name=?',
            (self.content, int(self.nsfw), self.guild.id, self.name),
        )
        db.conn.commit()
    ...
    def delete(self) -> None:
        ...
        db.cursor.execute(
            'DELETE FROM tags WHERE guild=? AND name=?',
...
class TagContext(TagSafeInteraction):
    ...
    def check_name(self, name: str) -> None:
        ...
        db.cursor.execute('SELECT guild FROM tags WHERE guild=? AND name=?', (self.guild_id, name))
        check: tuple[int] | None = db.cursor.fetchone()
    ...
    def get_member_tags(self, user: discord.Member) -> list[Tag]:
        ...
        db.cursor.execute('SELECT * FROM tags WHERE guild=? AND user=?', (self.guild_id, user.id))
    ...
    def get_guild_tags(self) -> list[Tag]:
        ...
        db.cursor.execute('SELECT * FROM tags WHERE guild=?', (self.guild_id,))
        tags: list[RawTag] = db.cursor.fetchall()
        return [Tag.from_db(self, tag) for tag in tags]
