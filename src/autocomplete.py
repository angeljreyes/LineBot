...
async def color(_: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
    color_options = list(filter(lambda x: x.startswith(current), list(core.colors_display)))
    color_options = [
        app_commands.Choice(name=core.colors_display[color], value=color) for color in core.colors
    ]
